...
vi.mock("@vscode/debugadapter", () => ({
  Handles: vi.fn().mockImplementation(function (base: number) {
      ...
      return {
        ...
        reset: vi.fn(function () { store.clear(); counter = base || 1 })
      }
    }),
...
vi.mock("../../services/funMessenger", () => ({
  funWindow: {
    withProgress: vi.fn(function (_opts: any, fn: () => any) { return fn() }),
...
vi.mock("./debugService", () => ({
  idThread: vi.fn(function (frameId: number) { return Math.floor(frameId / 1000000000000) }),
...
function makeListener(serviceMap: Map<number, any> = new Map()) {
  return {
    ...
    activeServices: vi.fn(function () { return [...serviceMap] })
  } as any
}
...
describe("VariableManager", () => {
  beforeEach(() => {
    vi.clearAllMocks()
    mockIdThread.mockImplementation(function (frameId: number) { return Math.floor(frameId / 1000000000000) })
  })
  ...
  describe("getScopes", () => {
    test("returns empty array when client throws", async () => {
      ...
      mockIdThread.mockReturnValue(threadId)
      const listener = {
        service: vi.fn().mockImplementation(function () { throw new Error("no service") }),
        activeServices: vi.fn(function () { return [] })
      } as any
      const vm = new VariableManager(listener)
    ...
    test("calls debuggerChildVariables with @ROOT", async () => {
      ...
      const listener = {
        service: vi.fn(function () { return service }),
        activeServices: vi.fn(function () { return [...serviceMap] })
      } as any
      const vm = new VariableManager(listener)
      vm.resetHandle(threadId)
    ...
    test("always adds SY scope", async () => {
      ...
      const listener = {
        service: vi.fn(function () { return service }),
        activeServices: vi.fn(function () { return [[threadId, service]] })
      } as any
      const vm = new VariableManager(listener)
      vm.resetHandle(threadId)
  ...
  describe("dumpJson", () => {
    ...
    test("returns numeric value for numeric TECHNICAL_TYPE", async () => {
      ...
      const listener = { service: vi.fn(), activeServices: vi.fn(function () { return [] }) } as any
      const vm = new VariableManager(listener)
