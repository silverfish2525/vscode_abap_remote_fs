...
const { mockUriRevisions, mockRevLabel } = vi.hoisted(() => {
  ...
  const mockRevLabel = vi.fn(function (rev: any, def: string) { return rev?.versionTitle || def })
  return { mockUriRevisions, mockRevLabel }
})
vi.mock("./abaprevisionservice", () => ({
  AbapRevisionService: { get: vi.fn(function () { return ({ uriRevisions: mockUriRevisions }) }) },
