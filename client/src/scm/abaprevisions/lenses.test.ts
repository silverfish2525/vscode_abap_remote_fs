...
const mockRevLabel = vi.fn((rev: any, def: string) => rev?.versionTitle || def);
vi.mock("./abaprevisionservice", () => ({
  AbapRevisionService: { get: vi.fn(() => ({ uriRevisions: mockUriRevisions })) },
...
describe.skip("provideCodeLenses", () => {
  ...
  it("quickdiff lens title includes current revision label", async () => {
    ...
    const lens = AbapRevisionLens.get();
    const result = await lens.provideCodeLenses({
      uri: { scheme: "adt", authority: "dev" },
    } as any);
