...
vi.mock("./scm", () => ({
  scmKey: vi.fn(function (auth: string, key: string) { return `abapGit_${auth}_${key}` }),
  scmData: vi.fn()
}))

vi.mock("../../lib", () => ({
  atob: vi.fn(function (s: string) { return Buffer.from(s, "base64").toString("utf-8") }),
...
describe("GitDocProvider.provideTextDocumentContent", () => {
  ...
  it("throws for non-ABAPGIT scheme URIs", async () => {
    ...
    const registeredProvider = ((__$mock_vscode).workspace.registerTextDocumentContentProvider as Mock).mock.calls[0][1]
