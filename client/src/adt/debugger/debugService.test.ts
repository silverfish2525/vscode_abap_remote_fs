...
vi.mock(
  ...
  () => ({
    ...
    Disposable: vi.fn(function (fn: any) {
      return { dispose: fn };
    }),
  }),
);
vi.mock("@vscode/debugadapter", () => ({
  ...
  StoppedEvent: vi.fn(function (reason: string, threadId: number) {
    return { type: "stopped", reason, threadId };
  }),
  ThreadEvent: vi.fn(function (reason: string, threadId: number) {
    return { type: "thread", reason, threadId };
  }),
