vi.mock(
  ...
  () => ({
    ...
    Position: vi
      .fn()
      .mockImplementation(function (line: number, char: number) { return ({ line, character: char }) }),
    Range: vi.fn().mockImplementation(function (start: any, end: any) { return ({ start, end }) }),
    CodeLens: vi.fn().mockImplementation(function (range: any, cmd: any) { return ({ range, command: cmd }) })
  })
)
