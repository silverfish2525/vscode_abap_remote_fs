...
test(
  "find object in $TMP",
  runTest(async (root) => {
    const abapgit = await root.findByAdtUri("/sap/bc/adt/programs/programs/zabapgit");
    ...
    expect(abapgit?.path).toBe("/$TMP/$ABAPGIT/Source Code Library/Programs/ZABAPGIT");
  }),
);
...
test(
  ...
  runTest(async (root) => {
    const abapgit = await root.findByAdtUri("/sap/bc/adt/programs/programs/zabapgit", true);
    expect(abapgit).toBeDefined();
    if (!isAbapStat(abapgit?.file)) fail("Abap Object expected");
    expect(abapgit?.file.object.name).toBe("ZABAPGIT");
    ...
    if (!isAbapFile(abapgit?.file)) fail("Leaf node expected for main program include");
...
test(
  ...
  runTest(async (root) => {
    const tabledesc = await root.findByAdtUri("/sap/bc/adt/oo/classes/cl_abap_tabledescr", true);
    ...
    if (!isAbapFile(tabledesc?.file)) fail("Leaf node expected for main program include");
    const source = await tabledesc?.file.read();
    expect(source?.match(/class\s+CL_ABAP_TABLEDESCR\s+definition/i)).toBeTruthy();
  }),
);
