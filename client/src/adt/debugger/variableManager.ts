...
const isDebugVariable = (v: DebugVariable | { id: string; name: string }): v is DebugVariable =>
  "ID" in v;
...
export class VariableManager {
  ...
  createVariable(threadId: number, v: DebugVariable | { id: string; name: string }) {
    if (isDebugVariable(v))
      return this.variableHandles(threadId).create({
        id: v.ID,
        name: v.NAME,
        lines: v.TABLE_LINES,
        meta: v.META_TYPE,
  ...
  private async childVariables(parent: Variable) {
    ...
    if (parent.meta === "table") {
      ...
      const keys = [...Array(parent.lines).keys()].map(
        (k) => `${parent.id.replace(/\[\]$/, "")}[${k + 1}]`,
      );
  ...
  async dumpJson(client: ADTClient, name: string | DebugVariable) {
    const v =
      typeof name !== "string" ? name : await client.debuggerVariables([name]).then((v) => v[0]);
    if (!v) return;
    switch (v.META_TYPE) {
      case "simple":
        if (v.TECHNICAL_TYPE.match(/B|S|I|INT8|P|N|DECFLOAT16|DECFLOAT34|F/))
  ...
  private static async exportJson(arg: {
