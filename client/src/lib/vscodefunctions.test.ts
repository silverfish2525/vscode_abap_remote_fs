vi.mock("vscode", () => {
  ...
  const Uri = {
    parse: vi.fn(function (s: string) { return ({
          ...
          with: vi.fn(function (this: any, changes: any) { return { ...this, ...changes } })
        }) })
  }
...
vi.mock("../services/funMessenger", () => ({
  funWindow: {
    withProgress: vi.fn(function (_opts: any, cb: any) { return cb() }),
...
vi.mock("./rfsTaskEither", () => ({
  rfsTryCatch: vi.fn(function (fn: any) { return fn })
}))

vi.mock("./functions", () => ({
  splitAdtUriInternal: vi.fn(),
  isUnDefined: vi.fn(function (x: any) { return x === undefined }),
  isFn: vi.fn(function (x: any) { return typeof x === "function" }),
  isNonNullable: vi.fn(function (x: any) { return x !== null && x !== undefined }),
  caughtToString: vi.fn(function (e: any) { return String(e) }),
  isString: vi.fn(function (x: any) { return typeof x === "string" })
}))
...
describe("vscPosition", () => {
  beforeEach(() => {
    ;(Position as Mock).mockImplementation(function (line: number, char: number) { return ({ line, character: char }) })
  })
...
describe("rangeApi2Vsc", () => {
  beforeEach(() => {
    ;(Position as Mock).mockImplementation(function (line: number, char: number) { return ({ line, character: char }) })
    ;(Range as Mock).mockImplementation(function (start: any, end: any) { return ({ start, end }) })
  })
...
describe("createStore", () => {
  const makeMockMemento = (initial?: [string, any][]) => {
    ...
    return {
      keys: vi.fn(function () { return stored.map(([k]) => k) }),
      get: vi.fn(function (key: string) { return stored.find(([k]) => k === key)?.[1] }),
...
describe("withp", () => {
  ...
  it("calls window.withProgress with correct options", async () => {
    ...
    ;(window.withProgress as Mock).mockImplementation(function (_opts: any, fn: any) { return fn() })
