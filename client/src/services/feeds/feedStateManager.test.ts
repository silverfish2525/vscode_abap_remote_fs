...
function makeContext(storagePath = "/storage") {
  ...
  return {
    ...
    globalState: {
      get: vi.fn(function (key: string) { return store[key] }),
...
describe("FeedStateManager construction", () => {
  ...
  test("loads entries from file when it exists", () => {
    const ctx = makeContext()
    ;(fsMock.existsSync as Mock).mockImplementation(function (p: any) {
  ...
  test("uses fallback values for missing entry fields", () => {
    const ctx = makeContext()
    ;(fsMock.existsSync as Mock).mockImplementation(function (p: any) { return String(p).includes("feedEntries.json") }
    )
    const stored = {
