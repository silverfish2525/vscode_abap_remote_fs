...
vi.mock("./rfsTaskEither", () => ({
  rfsTryCatch: vi.fn((fn: any) => fn)
}))

vi.mock("./functions", () => ({
  splitAdtUriInternal: vi.fn(),
  isUnDefined: vi.fn((x: any) => x === undefined),
  isFn: vi.fn((x: any) => typeof x === "function"),
  isNonNullable: vi.fn((x: any) => x !== null && x !== undefined),
...
describe("createStore", () => {
  const makeMockMemento = (initial?: [string, any][]) => {
    ...
    return {
      keys: vi.fn(() => stored.map(([k]) => k)),
      get: vi.fn((key: string) => stored.find(([k]) => k === key)?.[1]),
...
describe("withp", () => {
  ...
  it("calls window.withProgress with correct options", async () => {
    ...
    ;(window.withProgress as Mock).mockImplementation((_opts: any, fn: any) => fn())
