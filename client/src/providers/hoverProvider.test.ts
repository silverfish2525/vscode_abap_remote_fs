...
const makeMockDocument = (lines: string[], uriStr = "file:///test.abap") => {
  ...
  return {
    uri: mockUri,
    lineAt: vi.fn((lineOrPos: number | any) => {
      const line = typeof lineOrPos === "number" ? lineOrPos : lineOrPos.line
      return { text: lines[line] ?? "" }
    }),
    getText: vi.fn((range?: any) => lines.join("\n")),
  ...
  describe("error handling", () => {
    it("catches errors in provideHover and returns undefined", async () => {
      const doc = {
        ...
        lineAt: vi.fn(() => { throw new Error("document error") }),
        getText: vi.fn(),
        getWordRangeAtPosition: vi.fn(() => new vscode.Range(new vscode.Position(0, 0), new vscode.Position(0, 5)))
      } as any
