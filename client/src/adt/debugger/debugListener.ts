...
export const errorType = (err: any): string | undefined => {
  try {
    ...
    if (!exceptionType && `${err.response.body}`.match(/Connection timed out/))
...
export class DebugListener {
  ...
  private async hasConflict(): Promise<ConflictResult> {
    ...
    try {
      ...
      if (isConflictError(error))
        return { with: "other", message: error?.properties?.conflictText };
      throw error;
    }
    try {
  ...
  public async logout() {
    ...
    await this.stopListener().catch((e) =>
      log(`logout: stopListener failed: ${caughtToString(e)}`),
    );
