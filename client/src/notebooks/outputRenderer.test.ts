vi.mock("vscode", () => {
  const mockCellOutputItem = {
    text: vi.fn(function (value: string, mime: string) { return ({ data: Buffer.from(value), mime }) }),
    json: vi.fn(function (value: unknown) { return ({ data: Buffer.from(JSON.stringify(value)), mime: "application/json" }) }),
    error: vi.fn(function (err: any) { return ({ data: Buffer.from(JSON.stringify(err)), mime: "application/vnd.code.notebook.error" }) }),
  }
  const mockNotebookCellOutput = vi.fn().mockImplementation(function (items: any[]) { return ({ items }) })
  return {
...
describe("renderSqlOutput", () => {
  ...
  test("returns text output for empty result array", () => {
    ...
    const { NotebookCellOutputItem } = (__$mock_vscode)
