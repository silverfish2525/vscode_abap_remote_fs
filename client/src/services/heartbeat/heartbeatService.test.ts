...
vi.mock("vscode", () => {
  ...
  return {
    workspace: {
      getConfiguration: vi.fn(function () { return mockConfigObj }),
      onDidChangeConfiguration: vi.fn(function () { return ({ dispose: vi.fn() }) })
    },
    window: {
      createStatusBarItem: vi.fn(function () { return mockStatusBarItem }),
    ...
    CancellationTokenSource: vi.fn(function () { return ({
...
vi.mock("../funMessenger", () => ({
  funWindow: {
    createStatusBarItem: vi.fn(function () { return ({
          show: vi.fn(),
          hide: vi.fn(),
          dispose: vi.fn(),
          text: "",
...
describe("HeartbeatService start / stop", () => {
  ...
  test("does not start when enabled=false in config", async () => {
    const vscode = (__$mock_vscode)
    vscode.workspace.getConfiguration.mockReturnValue({
      get: vi.fn(function (key: string, def: any) { return key === "enabled" ? false : def })
    })
    const svc = initializeHeartbeatService(context)
    await svc.start()
