...
const toDecoration = (m: AtcFind): DecorationOptions => ({
  range: new Range(m.start, m.start),
  hoverMessage: m.finding.messageTitle,
});
...
function updateDecorations() {
  ...
  const infos = markers
    .filter((m) => !hasExemption(m.finding) && m.finding.priority !== 2 && m.finding.priority !== 1)
    .map(toDecoration);
  const warnings = markers
    .filter((m) => !hasExemption(m.finding) && m.finding.priority === 2)
    .map(toDecoration);
  const errors = markers
    .filter((m) => !hasExemption(m.finding) && m.finding.priority === 1)
    .map(toDecoration);
