...
export const rfsTaskEither =
  <T>(x: T): RfsTaskEither<T> =>
  async () =>
    nullToNone(x);
export const rfsTryCatch = <T>(f: Lazy<Promise<T | undefined>>): RfsTaskEither<T> => {
  const x = tryCatch(f, (e) =>
    types.isNativeError(e) ? e : new Error(`${(e as any)?.message || e}`),
  );
  return chainEitherK(nullToNone)(x);
};
export const rfsWrap =
  <T, R>(f: (x: T) => R | Promise<R>) =>
  (x: T) =>
    rfsTryCatch(async () => f(x));
export const rfsChainE = <P, T>(f: (x: P) => T | Promise<T>) => chain(rfsWrap(f));
export const rfsBind = <N extends string, A, B>(
  n: Exclude<N, keyof A>,
  f: (x: A) => B | Promise<B>,
) => bind(n, rfsWrap(f));

export const rfsBindReplace = <N extends string, A, B>(name: N, f: (x: A) => B | Promise<B>) =>
  chain((a: A) =>
    pipe(
      rfsWrap(f)(a),
      map((value) => {
...
export const addField =
  <K extends string, P, R>(name: K, f: (x: P) => Promise<R>) =>
...
export const createTaskTransformer =
  <T>(f: (y: T) => T | Option<T> | Promise<T | Option<T>>) =>
...
export const chainTaskTransformers =
  <T>(first: TaskTransformer<T>, ...rest: TaskTransformer<T>[]) =>
  (y: T) =>
    rest.reduce(taskEither.chain, first(y));
