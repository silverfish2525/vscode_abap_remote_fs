vi.mock("@vscode/debugadapter", () => ({
  ...
  InitializedEvent: vi.fn().mockImplementation(function () { return ({ type: "initialized" }) }),
  StoppedEvent: vi.fn().mockImplementation(function (reason: string, threadId: number) { return ({ type: "stopped", reason, threadId }) }),
  TerminatedEvent: vi.fn().mockImplementation(function () { return ({ type: "terminated" }) }),
  Thread: vi.fn().mockImplementation(function (id: number, name: string) { return ({ id, name }) }),
  Source: vi.fn().mockImplementation(function (name: string, path: string) { return ({ name, path }) })
}))
vi.mock("abap-adt-api", () => ({
  debugMetaIsComplex: vi.fn(function (meta: string) { return ["structure", "table", "object", "class"].includes(meta) }
  )
}))
...
vi.mock("./replayVariableManager", () => {
  return {
    ReplayVariableManager: vi.fn().mockImplementation(function () { return ({
          reset: vi.fn(),
