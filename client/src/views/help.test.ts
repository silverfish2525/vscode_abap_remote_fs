...
vi.mock("./utilities", () => ({
  injectUrlHandler: vi.fn(function (html: string) { return html + "<!-- injected -->" }),
}))
...
describe("showAbapDoc", () => {
  ...
  it("returns early if document scheme is not adt", async () => {
    const fakeEditor = {
      document: {
        ...
        getText: vi.fn(function () { return "code" }),
      },
  ...
  it("opens webview panel with documentation", async () => {
    ...
    const fakeEditor = {
      document: {
        ...
        getText: vi.fn(function () { return "WRITE 'hello'." }),
      },
  ...
  it("handles webview message with adt url scheme", async () => {
    ...
    const mockPanel = {
      webview: {
        html: "",
        onDidReceiveMessage: vi.fn(function (cb: any) { messageHandlers.push(cb) }),
        postMessage: vi.fn(),
      },
    }
    ;(mockedWindow as any).createWebviewPanel = vi.fn().mockReturnValue(mockPanel)
    ...
    ;(AdtObjectFinder as Mock).mockImplementation(function () { return ({ displayAdtUri: mockDisplayAdtUri }) })

    const { Uri } = (__$mock_vscode)
    ;(Uri.parse as Mock).mockReturnValueOnce({ scheme: "adt", toString: () => "adt://x/y", authority: "", path: "", query: "" })
