...
vi.mock("../../services/funMessenger", () => ({
  funWindow: {
    withProgress: vi.fn((_opts: any, fn: () => any) => fn()),
...
vi.mock("./debugService", () => ({
  idThread: vi.fn((frameId: number) => Math.floor(frameId / 1000000000000)),
...
function makeListener(serviceMap: Map<number, any> = new Map()) {
  return {
    service: vi.fn((id: number) => {
      const svc = serviceMap.get(id)
      if (!svc) throw new Error(`No service for threadid ${id}`)
      return svc
    }),
    activeServices: vi.fn(() => [...serviceMap])
  } as any
}
...
describe("VariableManager", () => {
  ...
  describe("getScopes", () => {
    test("returns empty array when client throws", async () => {
      ...
      mockIdThread.mockReturnValue(threadId)
      const listener = {
        service: vi.fn().mockImplementation(() => { throw new Error("no service") }),
        activeServices: vi.fn(() => [])
      } as any
      const vm = new VariableManager(listener)
    ...
    test("always adds SY scope", async () => {
      ...
      const listener = {
        service: vi.fn(() => service),
        activeServices: vi.fn(() => [[threadId, service]])
      } as any
      const vm = new VariableManager(listener)
      vm.resetHandle(threadId)
  ...
  describe("dumpJson", () => {
    ...
    test("returns numeric value for numeric TECHNICAL_TYPE", async () => {
      ...
      const listener = { service: vi.fn(), activeServices: vi.fn(() => []) } as any
      const vm = new VariableManager(listener)
