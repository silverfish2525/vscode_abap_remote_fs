vi.mock("abap-adt-api", () => ({
  isDebuggerBreakpoint: vi.fn((bp: any) => bp && bp.__isDebuggerBP === true),
}));
vi.mock("abapfs", () => ({
  isAbapFile: vi.fn((node: any) => node && node.__isAbapFile === true),
}));
vi.mock(
  "vscode",
  () => ({
...
vi.mock("@vscode/debugadapter", () => ({
  ...
  Source: vi.fn(function (name: string, path: string) {
    return { name, path };
  }),
}));
vi.mock("../../lib", () => ({
...
function makeAbapNode(overrides: Partial<any> = {}) {
  return {
    ...
    object: {
      structure: {},
      contentsPath: vi.fn(() => "/sap/bc/adt/programs/programs/ZPROG"),
  ...
  describe("include breakpoint URI handling", () => {
    test("builds VIT URI for includes (with mainPrograms)", async () => {
      ...
      const node = makeAbapNode({
        object: {
          structure: {},
          contentsPath: vi.fn(() => "/sap/bc/adt/programs/includes/ZINCLUDE"),
          loadStructure: vi.fn().mockResolvedValue(undefined),
