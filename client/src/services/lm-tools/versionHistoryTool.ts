...
export class VersionHistoryTool implements vscode.LanguageModelTool<IVersionHistoryParameters> {
  ...
  async invoke(
    ...
    try {
      ...
      switch (action) {
        case "get_version_source":
          return await this.getVersionSource(
            revisions,
            versionNumber!,
            objectName,
            objectInfo.type,
        ...
        default:
          ...
          const resultText = this.formatVersionHistory(
            objectName,
            objectInfo.type,
            limitedRevisions,
            revisions.length,
  ...
  private async getVersionSource(
    ...
    if (versionNumber < 1 || versionNumber > revisions.length) {
      throw new Error(
        `Version ${versionNumber} not found. Available versions: 1 to ${revisions.length}`,
      );
    }
  ...
  private async compareVersions(
    ...
    if (version1 < 1 || version1 > revisions.length) {
      throw new Error(
        `Version ${version1} not found. Available versions: 1 to ${revisions.length}`,
      );
    }
    if (version2 < 1 || version2 > revisions.length) {
      throw new Error(
        `Version ${version2} not found. Available versions: 1 to ${revisions.length}`,
      );
    }
