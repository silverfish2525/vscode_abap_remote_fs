...
vi.mock("../../config", () => ({
  connectedRoots: vi.fn(function () { return new Map() })
}))
vi.mock("./feedParsers", () => ({
  parseFeedResponse: vi.fn(function () { return [] }),
  toFeedMetadata: vi.fn(function (f: any) { return f })
}))
vi.mock("abap-adt-api/build/utilities", () => ({
  fullParse: vi.fn(),
  xmlArray: vi.fn(function () { return [] })
}))
vi.mock("fs")
...
describe("handleUnavailableFeed warning notification", () => {
  ...
  test("shows warning when feed is unavailable for the first time", async () => {
    const { funWindow: w } = (__$mock_funMessenger)
