vi.mock("vscode", () => ({
  ...
  Uri: { parse: vi.fn(function (s: string) { return ({ toString: () => s }) }) },
  workspace: { openTextDocument: vi.fn() },
  Selection: vi.fn(function (start: any, end: any) { return ({ start, end }) }),
  WorkspaceEdit: vi.fn().mockImplementation(function () { return ({ insert: vi.fn() }) }),
  Position: vi.fn(function (line: number, character: number) { return ({ line, character }) })
}))
