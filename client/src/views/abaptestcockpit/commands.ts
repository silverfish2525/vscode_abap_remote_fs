...
const tryIgnoreFinfing = async (finding?: AtcFind) => {
  ...
  const pragma =
    pragmas.length === 1
      ? pragmas[0]
      : rfsExtract(await quickPick(pragmas, { placeHolder: "Select override" })());
  if (!pragma) return;
  const uriP = Uri.parse(finding.uri);
  const document = await workspace.openTextDocument(uriP);
...
const selectors = (connId: string) => ({
  ...
  inputReason: fieldReplacer(
    "reason",
    quickPick(
      [
        { label: "False Positive", value: "FPOS" },
  ...
  approver: (value?: string) =>
    fieldReplacer("approver", inputBox({ prompt: "Enter approver user ID", value })),
});
const requestExemption = async (item: AtcFind) => {
  try {
    ...
    const { inputReason, approver, notifyOn, inputJustification } = selectors(
      item.parent.parent.connectionId,
    );
    const actualResult = await chainTaskTransformers<AtcProposal>(
      inputReason,
      approver(aapprover),
      notifyOn,
      inputJustification,
...
const selectKey = <K extends string, T extends Record<K, boolean>>(
  keys: K[],
  r: T,
...
const requestExemptionAll = async (item: AtcFind) => {
  try {
    ...
    const target = rfsExtract(
      await selectKey(
        ["object", "package", "subobject"],
    ...
    const targetmsg = rfsExtract(
      await selectKey(["check", "message"], proposal.restriction.rangeOfFindings.restrictByCheck, {
        placeHolder: "Select target object",
      })(),
    );
    if (!targetmsg) return;
