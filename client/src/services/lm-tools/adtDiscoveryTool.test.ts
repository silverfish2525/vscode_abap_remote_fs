vi.mock("vscode", () => {
  ...
  return {
    ...
    Uri: {
      ...
      joinPath: vi.fn(function (...args: any[]) { return ({
  ...
  describe("invoke", () => {
    ...
    it("calls discovery endpoints in parallel via Promise.all", async () => {
      ...
      mockClient.adtDiscovery.mockImplementation(
        function () { return new Promise(r => setTimeout(() => { discoveryResolved = true; r(minimalDiscovery) }, 10)) }
      )
      mockClient.adtCoreDiscovery.mockImplementation(
        function () { return new Promise(r => setTimeout(() => { coreResolved = true; r(minimalCoreDiscovery) }, 10)) }
      )
