vi.mock("abap-adt-api", () => ({
  isAdtError: vi.fn(function (e: any) { return e && e.__isAdtError === true }),
...
vi.mock("vscode", () => ({
  EventEmitter: vi.fn().mockImplementation(function () {
      const listeners: any[] = []
      return {
        event: vi.fn(function (listener: any, _thisArg?: any, disposables?: any[]) {
                  listeners.push(listener)
                  const d = { dispose: vi.fn() }
                  if (Array.isArray(disposables)) disposables.push(d)
                  return d
                }),
        fire: vi.fn(function (e: any) { listeners.forEach(l => l(e)) }),
        dispose: vi.fn()
      }
    }),
  Disposable: vi.fn().mockImplementation(function (fn: any) { return ({ dispose: fn }) })
}))
vi.mock("@vscode/debugadapter", () => ({
  ContinuedEvent: vi.fn().mockImplementation(function (threadId: number) { return ({ type: "continued", threadId }) }),
  StoppedEvent: vi.fn().mockImplementation(function (reason: string, threadId: number) { return ({ type: "stopped", reason, threadId }) }),
  ThreadEvent: vi.fn().mockImplementation(function (reason: string, threadId: number) { return ({ type: "thread", reason, threadId }) }),
  Source: vi.fn().mockImplementation(function (name: string, path: string) { return ({ name, path }) })
}))
vi.mock("./functions", () => ({
