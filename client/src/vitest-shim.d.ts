...
//
// Goal: keep the mechanical jest -> vitest sed-rename in tests purely
// mechanical, without forcing an `import { ... } from "vitest"` into every
// test file.
//
...
// 2. We re-export jest's Mock / Mocked / MockedFunction / MockedClass /
//    MockedObject / MockInstance type aliases as globals (vitest only
//    exposes them as members of the `vitest` package).
// 3. We re-add `fail()`, which `@types/jest` declared globally and the tests
//    rely on in a handful of "expected branch" assertions.
