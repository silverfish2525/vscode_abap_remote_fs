...
vi.mock("fp-ts/lib/Either", () => ({
  isRight: vi.fn(function (v: any) { return v && v._tag === "Right" }),
}))

vi.mock("fp-ts/lib/Option", () => ({
  isNone: vi.fn(function (v: any) { return !v || v._tag === "None" }),
  none: { _tag: "None" },
  isSome: vi.fn(function (v: any) { return v && v._tag === "Some" }),
}))
...
vi.mock("vscode", () => ({
  ...
  workspace: {
    workspaceFolders: [],
    onDidChangeWorkspaceFolders: vi.fn(function () { return ({ dispose: vi.fn() }) }),
  },
  Uri: {
...
vi.mock("uuid", () => ({ v1: vi.fn(function () { return "test-uuid" }) }))
