...
describe("UnitTool", () => {
  ...
  describe("invoke", () => {
    test("returns unit test results as JSON", async () => {
      ...
      ;(vscode.window.withProgress as Mock).mockImplementation(async (_opts: any, fn: Function) => {
        return fn()
      })
    ...
    test("activates inactive object before running tests", async () => {
      ...
      ;(UnitTestRunner.get as Mock).mockReturnValue(mockRunner)
      ;(vscode.window.withProgress as Mock).mockImplementation(async (_opts: any, fn: Function) => fn())
