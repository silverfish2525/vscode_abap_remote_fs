...
const mockReadFile = vscode.workspace.fs.readFile as MockedFunction<typeof vscode.workspace.fs.readFile>
const mockWriteFile = vscode.workspace.fs.writeFile as MockedFunction<typeof vscode.workspace.fs.writeFile>
...
describe("TestDocumentCreator.createDocument", () => {
  ...
  it("adds error paragraph when image fails to load", async () => {
    ...
    const scenarios = [makeScenario(1, 1)]
    await creator.createDocument({ scenarios })
    const errorRun = (TextRun as Mock).mock.calls.find(
  ...
  it("screenshot description is included in paragraph text", async () => {
    ...
    await creator.createDocument({ scenarios })
    const descRun = (TextRun as Mock).mock.calls.find(
