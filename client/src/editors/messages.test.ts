...
vi.mock("vscode", () => ({
  workspace: {
    onDidChangeTextDocument: vi.fn(function () { return ({ dispose: vi.fn() }) }),
    fs: {}
  },
...
vi.mock("fast-xml-parser", () => ({
  XMLParser: vi.fn().mockImplementation(function () { return ({
      parse: vi.fn()
    }) })
}))
vi.mock("html-entities", () => ({ decode: vi.fn(function (s: string) { return s }) }))
vi.mock("path", () => ({ join: vi.fn(function (...args: string[]) { return args.join("/") }) }))
