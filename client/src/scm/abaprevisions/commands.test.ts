...
vi.mock("../../services/funMessenger", () => ({
  funWindow: {
    ...
    withProgress: vi.fn(function (_opts: any, cb: any) { return cb({}, { isCancellationRequested: false }) })
  }
}))
...
vi.mock("./abaprevisionservice", () => ({
  ...
  revLabel: vi.fn(function (rev: any, fallback: string) { return rev?.version || fallback })
}))

vi.mock("./documentprovider", () => ({
  decodeRevisioUrl: vi.fn(),
  revisionUri: vi.fn(function (uri: any, rev: any, norm?: boolean) { return ({
      ...uri,
      scheme: "adt_revision",
      revision: rev
    }) }),
...
vi.mock("../../config", () => ({
  ...
  formatKey: vi.fn(function (s: string) { return s.toLowerCase() })
}))
...
vi.mock("../../lib", () => ({
  ...
  atob: vi.fn(function (s: string) { return Buffer.from(s, "base64").toString() }),
  btoa: vi.fn(function (s: string) { return Buffer.from(s).toString("base64") })
}))
...
vi.mock("../../commands", () => ({
  ...
  command: vi.fn(function () { return vi.fn() })
}))
