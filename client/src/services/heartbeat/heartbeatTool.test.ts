...
vi.mock("vscode", () => {
  ...
  return {
    ...
    CancellationTokenSource: vi.fn(function () { return ({ token: {}, cancel: vi.fn() }) })
  }
})
...
vi.mock("./heartbeatWatchlist", () => ({
  HeartbeatWatchlist: {
    getAllTasks: vi.fn(function () { return [] }),
    getFilePath: vi.fn(function () { return "/workspace/heartbeat.json" }),
    addTask: vi.fn(),
    removeTask: vi.fn(),
    updateTask: vi.fn(),
    read: vi.fn()
...
describe("HeartbeatTool invoke - add_task", () => {
  ...
  test("hints to start heartbeat when service is not running", async () => {
    ...
    ;(getHeartbeatService as Mock).mockReturnValue(makeService({ getStatus: vi.fn().mockReturnValue({ isRunning: false, isPaused: false, stats: { totalRuns: 0, successfulRuns: 0, alerts: 0, errors: 0, skipped: 0, averageDurationMs: 0 } }) }))
