...
const removeObsolete = (parent: TestItem, ids: Set<string>) =>
  [...parent.children].filter((c) => !ids.has(c[0])).forEach((i) => parent.children.delete(i[0]));

const setMethodResult = (run: TestRun, meth: UtMethod, item: TestItem) => {
  const outcome = methodOutcome(meth);
  item.range = meth.range;
  if (outcome.passed) run.passed(item, meth.executionTime * 1000);
  else run.failed(item, outcome.messages, meth.executionTime * 1000);
};
...
const setObjectResult = (
  ...
  for (const c of classes) setClassResult(run, c, parent, ctrl);
...
const convertAlert = (alert: UnitTestAlert): TestMessage =>
  new TestMessage(new MarkdownString(`${alert.title}<br>${alert.details.join("<br>")}`, true));
...
const setResults = (
  ...
  switch (resType) {
    case TestResType.class:
      if (classes.length === 1) return setClassResult(run, classes[0]!, item, ctrl);
...
export class UnitTestRunner {
  ...
  setUrlTypes(classes: UtClass[]) {
    for (const c of classes) {
      ...
      for (const m of c.testmethods) this.urlTypes.set(m.uri, TestResType.method);
    }
  }
