...
export const renameHandler = async (
  ...
  const renameEvaluateResult = await co.client.statelessClone.renameEvaluate(
    co.obj.mainUrl,
    position.line + 1,
    position.character,
    position.character
  ...
  for (const obj of renameEvaluateResult.affectedObjects) {
    ...
    changes[uri] = obj.textReplaceDeltas.map(d => {
      const start: Position = {
        line: d.rangeFragment.start.line - 1,
        character: d.rangeFragment.start.column
      }
      const end: Position = {
        line: d.rangeFragment.end.line - 1,
        character: d.rangeFragment.end.column
      }
