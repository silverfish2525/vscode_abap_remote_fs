vi.mock("vscode", () => {
  ...
  const ThemeColor = vi.fn(function (id: string) { return ({ id }) })
  const ThemeIcon = vi.fn(function (id: string, color?: any) { return ({ id, color }) })
  return {
    Position,
    Range,
    ThemeColor,
...
vi.mock("ramda", () => ({
  sortWith: vi.fn(function () { return (arr: any[]) => arr }),
  ascend: vi.fn(function () { return vi.fn() }),
  prop: vi.fn(function () { return vi.fn() })
}))
...
describe("AtcFind", () => {
  ...
  it("applyEdits updates start position for edits before current line", () => {
    const finding = makeFinding()
    const parent = makeParent()
    const MockPos = function (this: any, line: number, char: number) { Object.assign(this, { line, character: char }) }
