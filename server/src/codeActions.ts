...
const createExtractmethod = (parms: CodeActionParams) =>
  CodeAction.create(
    "Extract method",
    Command.create("extractMethod", "abapfs.extractMethod", parms.textDocument.uri, parms.range),
...
async function quickfix(parms: CodeActionParams): Promise<CodeAction[] | undefined> {
  try {
    ...
    for (const d of diag)
      if (d.severity === DiagnosticSeverity.Error || d.severity === DiagnosticSeverity.Warning) {
        ...
        try {
          ...
          for (const p of proposals) if (shouldAdd(p, allProposals)) allProposals.push(p);
        } catch (e) {
          log(e);
        }
      }
