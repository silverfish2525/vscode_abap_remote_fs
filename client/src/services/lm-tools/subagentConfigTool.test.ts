vi.mock("vscode", () => ({
  ...
  lm: {
    registerTool: vi.fn(function () { return ({ dispose: vi.fn() }) }),
    onDidChangeChatModels: vi.fn(function () { return ({ dispose: vi.fn() }) })
  },
...
vi.mock("../subagentRegistry", () => ({
  ...
  getSubagentSettings: vi.fn(function () { return ({ enabled: false, models: {} }) }),
  getWorkspaceFolder: vi.fn(),
  getAvailableModels: vi.fn(function () { return [] }),
  getExtensionId: vi.fn(function () { return "test.extension" }),
  validateModelConfiguration: vi.fn(function () { return [] }),
  buildFullToolName: vi.fn(function (ext: string, tool: string) { return `${ext}#${tool}` })
}))
